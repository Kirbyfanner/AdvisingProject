...
<html lang="en">
  ...
  <body>
      ...
      <div id="form">
        <div class="top">
          ...
          <?php
			...
			//Fetch the appointment being deleted
            $sql = "SELECT `id` FROM `Proj2Advisors` WHERE `FirstName` = '$row[1]' AND `LastName` = '$row[2]'";
            $rs = $COMMON->executeQuery($sql, "Advising Appointments");
            $rod = mysql_fetch_row($rs);
            $adv = $rod[0];

			//Get the student's information that had signed up for this appointment
            if($row[4]){
			...
			//Display the removed appointment
			...
			//Let the student know that the appointment was deleted
            if($row[4]){
              echo("$std</b>");
	...
	<div class="bottom">
		...
		//Let the advisor know that the student was notified that their appointment was cancelled
		if($row[4]){
